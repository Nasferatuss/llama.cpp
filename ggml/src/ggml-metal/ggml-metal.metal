...
void kernel_mul_mv_tq2_0_f32_impl(
    ...
    // 16 threads per block, NBLOCK blocks per pass
    constexpr short NBLOCK = 2;

    constexpr short NB = N_SIMDWIDTH/NBLOCK; // threads per block

    const short blk = tiisg / NB;    // 0..NBLOCK-1, block handled by this thread
    const short tg  = tiisg % NB;    // 0..NB-1, thread within block
    const short h0  = tg / 8;        // 0 or 1: first/second half of the block
    const short htg = tg % 8;        // 0..7

    // byte and y base offsets within the block (16 elements, 4 per byte)
    const short base = 4*htg + 128*h0;

    device const float4 * yb4 = (device const float4 *)(y + base + blk*QK_K);

    // Hoisted per-byte coefficients (from y) and total y-sum, shared across rows:
    //   sum (f_j - 1)*y_j = Y0[j]*v + (Y1[j]-4*Y0[j])*floor(v/4)
    //                      + (Y2[j]-4*Y1[j])*floor(v/16) + (Y3[j]-4*Y2[j])*floor(v/64)
    //                      - sum_{j,k} Y_k[j]
    float4 coef[4];

    for (int ib = blk; ib < nb; ib += NBLOCK) {
        const float4 Y0 = yb4[ 0];
        const float4 Y1 = yb4[ 8];
        const float4 Y2 = yb4[16];
        const float4 Y3 = yb4[24];

        float sumy = 0.f;
        FOR_UNROLL (short j = 0; j < 4; ++j) {
            coef[j] = float4(Y0[j], Y1[j] - 4.0f*Y0[j], Y2[j] - 4.0f*Y1[j], Y3[j] - 4.0f*Y2[j]);
        ...
        FOR_UNROLL (short row = 0; row < nr0; ++row) {
            ...
            FOR_UNROLL (short j = 0; j < 4; ++j) {
                ...
                sum += coef[j][0]*v + coef[j][1]*f1 + coef[j][2]*f2 + coef[j][3]*f3;
            }
